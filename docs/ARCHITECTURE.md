...
| SQL 多跳无过滤      | `traverse_chain_sql` 单次 JOIN（`edge_joinable_sql`） | **DB**（fast path） |
| SQL 多跳不可 JOIN  | `can_traverse_chain_sql` 为假或 JOIN 失败 → `_hop_neighbors_batch` | **Python 逐跳**     |
...
### 14.6 复杂边与 Gremlin 写路径


| 主题             | 框架行为                                                                                                                                    |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| 复杂边 SQL        | `edge_joinable_sql` / `can_traverse_chain_sql` 判定每跳是否可 JOIN；不可则 `TraverseChain` 走 `_hop_neighbors_batch` 逐跳；`traverse_chain_sql` 若抛 `ValueError` 亦自动回退 |
| Gremlin Update | `graph_ops.update_node` 返回最新 `dict`：有字段更新时 `property(...).valueMap(True)` 同次遍历；仅改边时一次 `valueMap` 读取；`save()` 不再调用 `get_by_id` |
